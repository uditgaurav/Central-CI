...
var _ = Describe("BDD of container-kill experiment", func() {
	...
	Context("Check for Litmus components", func() {
		...
		It("Should check for creation of runner pod", func() {
			...
			err := utils.InstallRbac(chaosTypes.ContainerKillRbacPath, chaosTypes.ChaosNamespace, experimentName, client)
			Expect(err).To(BeNil(), "Fail to create RBAC")
			klog.Info("Rbac has been created successfully !!!")

			//Creating Chaos-Experiment
			By("Creating Experiment")
			err = utils.DownloadFile(experimentName+".yaml", chaosTypes.ContainerKillExperimentPath)
			...
			err = utils.DownloadFile(experimentName+"-ce.yaml", chaosTypes.ContainerKillEnginePath)
			...
			err = utils.EditKeyValue(experimentName+"-ce.yaml", "TARGET_CONTAINER", "value: 'nginx'", "value: '"+chaosTypes.TargetContainer+"'")
